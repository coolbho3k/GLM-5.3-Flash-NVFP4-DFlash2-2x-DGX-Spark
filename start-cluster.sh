...
if [[ -n "${EXL3_FAT_ACTIVATION_CONTROL:-}" ]]; then
  ...
  activation_control_host="$(dirname -- "$ADAPTIVE_SCHEDULER_CONFIG")/exl3-activation.json"
  test -s "$activation_control_host"
  ssh "${ssh_opts[@]}" "$WORKER_HOST" "mkdir -p $(printf '%q' "$REMOTE_DIR/scheduler_profiles")"
  scp "${ssh_opts[@]}" "$activation_control_host" "$WORKER_HOST:$REMOTE_DIR/scheduler_profiles/exl3-activation.json"
fi
