...
def decode_records(records: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
...
def main() -> None:
    ...
    concat_and_cache_nvfp4_mla_zero_rope(source, cache, slots)
    torch.cuda.synchronize()

    records = cache[0, : args.tokens]
    reconstructed, scales = decode_records(records)
    ...
    for batch_size in batch_sizes:
        ...
        def write() -> None:
            concat_and_cache_nvfp4_mla_zero_rope(
                source[:batch_size], cache, slots[:batch_size]
            )
