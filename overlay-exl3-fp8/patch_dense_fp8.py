...
MLA_PASSTHROUGH_OLD = """                quant_config=quant_config,
                prefix=f"{prefix}.self_attn",
"""
MLA_PASSTHROUGH_NEW = """                quant_config=quant_config,  # [glm53-dense-fp8]
                prefix=f"{prefix}.self_attn",
"""
...
def patch_mla(path: Path) -> None:
    text = path.read_text()
    if MARK in text:
        print(f"{path.name}: {MARK} already present — skipping")
        return
    if text.count(MLA_OLD) == 1:
        new_text = text.replace(MLA_OLD, MLA_NEW, 1)
        label = "mla quant_config"
    elif text.count(MLA_PASSTHROUGH_OLD) == 1:
        new_text = text.replace(MLA_PASSTHROUGH_OLD, MLA_PASSTHROUGH_NEW, 1)
        label = "mla quant_config already passed through"
    else:
        raise SystemExit(f"{path}: expected one supported mla quant_config target")
