#!/usr/bin/env bash
# Build EXL3 + native zero-RoPE FP8 KV + DCP2 from pinned public inputs.
set -Eeuo pipefail
...
SPARKINFER_PATCH="$SCRIPT_DIR/vendor/sparkinfer-glm53-fp8-zero-rope.patch"
...
IMAGE="${IMAGE:-glm53-exl3:e2-fp8-dcp2}"
LOCAL_SPARKINFER_DEFAULT="/home/emi/code/glm/vendor/sparkinfer"
BUILD_ROOT="$(mktemp -d -t glm53-exl3-fp8-build.XXXXXXXX)"
SPARKINFER_CONTEXT="$BUILD_ROOT/sparkinfer"
WORKTREE_OWNER=""

cleanup() {
  if [[ -n "$WORKTREE_OWNER" && -e "$SPARKINFER_CONTEXT/.git" ]]; then
    git -C "$WORKTREE_OWNER" worktree remove --force "$SPARKINFER_CONTEXT" >/dev/null 2>&1 || true
  fi
  rm -rf -- "$BUILD_ROOT"
}
...
if [[ -n "$source_repo" ]]; then
  ...
  git -C "$source_repo" worktree add --detach "$SPARKINFER_CONTEXT" "$SPARKINFER_COMMIT"
...
docker build \
  ...
  --file "$SCRIPT_DIR/overlay-exl3-fp8/Dockerfile" \
  --tag "$IMAGE" \
  "$SCRIPT_DIR"
