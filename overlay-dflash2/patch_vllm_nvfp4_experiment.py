...
def replace(path: str, old: str, new: str) -> None:
    ...
    if new in text:
        return
    if old not in text:
        raise RuntimeError(f"patch anchor missing in {target}: {old[:120]!r}")
...
''',
    '''            # FlashInfer can split this non-prefix draft page at 32-token
            ...
            # the group is excluded from prefix hashing and scheduler LCM below.
            and fit_block % 32 == 0
            and len(draft_specs) <= len(mla_names)
''',
)
...
''',
    '''        # Allocation-only groups (for example the exact-fit DFlash
        # sliding-window cache) deliberately do not participate in prefix
        # hashing. They still allocate, recycle, and free blocks normally,
        # but there is nothing to publish into the shared prefix-cache map.
