...
configure_profile() {
  ...
  case "$profile" in
    ...
    exl3-fp4-dcp2)
      ...
      ;;
    exl3-fp8-dcp2)
      default_var IMAGE glm53-exl3:e3-fp8-dcp2
      default_var MODEL_HOST_PATH "$EXL3_MODEL_DEFAULT"
      default_var QUANTIZATION exl3
      default_var MOE_BACKEND auto
      default_var VLLM_ATTENTION_BACKEND FLASHINFER_MLA_SPARSE_SM120
      ...
      # MiaAI E3 grouped prefill; 64 rows keeps C1-C6 K=7 decode in the fused path.
      default_var EXL3_FAT_GROUPED 1
      default_var EXL3_TEMP_ROWS_FUSED 64
      default_var MAX_NUM_BATCHED_TOKENS 7168
      default_var SYNC_IMAGE_TO_WORKER 1
      ;;
    *)
  ...
  default_var EXL3_FAT_GROUPED 0
  default_var EXL3_FAT_PIPELINE off
  default_var EXL3_FAT_PIPELINE_CONTROL ""
  if [[ -n "$EXL3_FAT_PIPELINE_CONTROL" ]]; then
    [[ "$profile" == exl3-fp8-dcp2 && "$PREFILL_ADMISSION_POLICY" == adaptive && "$EXL3_FAT_KERNEL" == 1 && "$EXL3_FUSED_MOE" == 1 && "$EXL3_MOE_ROW_TILE" == 0 ]] || {
      echo "EXL3_FAT_PIPELINE_CONTROL requires adaptive exl3-fp8-dcp2 with fused E2 and no row-tile override" >&2
      exit 2
    }
  fi
  ...
  if [[ "$EXL3_FAT_GROUPED" == 1 ]]; then
    [[ "$profile" == exl3-fp8-dcp2 && "$EXL3_FUSED_MOE" == 1 && "$EXL3_MOE_ROW_TILE" == 0 ]] || {
      echo "EXL3_FAT_GROUPED=1 requires exl3-fp8-dcp2, fused MoE, and EXL3_MOE_ROW_TILE=0" >&2
      exit 2
    }
  elif [[ "$EXL3_FAT_GROUPED" != 0 ]]; then
    echo "EXL3_FAT_GROUPED must be 0 or 1" >&2
    exit 2
  fi
  default_var GLM53_EXL3_MOE_FAST 0
  default_var GLM53_EXL3_MOE_STREAM_WEIGHTS 0
  case "$GLM53_EXL3_MOE_STREAM_WEIGHTS" in
    0) ;;
    1)
      if [[ "$profile" != exl3-fp8-dcp2 || "$GLM53_EXL3_MOE_FAST" != 1 || "$EXL3_FUSED_MOE" != 1 ]]; then
        echo "Streaming-weight experiment requires exl3-fp8-dcp2, GLM53_EXL3_MOE_FAST=1 and EXL3_FUSED_MOE=1" >&2
        exit 2
      fi
      ;;
    *) echo "GLM53_EXL3_MOE_STREAM_WEIGHTS must be 0 or 1" >&2; exit 2 ;;
  esac
