...
# Schema 2 adds the grouped tier and its kernel/scratch diagnostics.
...
def grouped_fat_enabled() -> bool:
    """Enable MiaAI E3 grouped fat-expert prefill kernels (default off)."""
    return os.environ.get("EXL3_FAT_GROUPED", "0") != "0"


def fat_expert_log_enabled() -> bool:
    # E3 avoids a host sync; keep its optional routing histogram off by default.
    default = "0" if grouped_fat_enabled() else "1"
    return os.environ.get("EXL3_FAT_EXPERT_LOG", default) != "0"

def configured_fat_tier() -> str:
    """Highest fat tier requested: grouped > kernel > batched > sorted > legacy."""
...
def resolve_exl3_fat_tier(
    ...
    if configured == "grouped":
        ...
        if not fat_moe_symbols:
            raise RuntimeError(
                "EXL3_FAT_GROUPED=1 requires the grouped E3 kernel symbols; "
                "rebuild this image or set EXL3_FAT_GROUPED=0"
            )
        if grouped_eligible is None:
            grouped_eligible = (True, "eligible")
        if grouped_eligible[0]:
            return "grouped", "grouped_ok"
        configured = "kernel"
...
class Exl3MoEMethod(FusedMoEMethodBase):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        # E3 eligibility needs the constructed LinearEXL3 handles.
